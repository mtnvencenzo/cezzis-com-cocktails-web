...
export const hasAuthParams = (searchParams = window.location.search): boolean => (CODE_RE.test(searchParams) || ERROR_RE.test(searchParams)) && STATE_RE.test(searchParams);
...
const normalizeErrorFn =
    ...
    (error: unknown): Error => {
        ...
        if (error !== null && typeof error === 'object' && 'error' in error && typeof (error as WithError).error === 'string') {
            if ('error_description' in error && typeof (error as WithErrorAndDescription).error_description === 'string') {
