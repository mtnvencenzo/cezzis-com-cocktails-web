...
export interface Auth0ProviderOptions<TUser extends User = User> extends Auth0ClientOptions {
    ...
    onClientCreated?: (client: Auth0Client) => Auth0Client;
...
const Auth0Provider = <TUser extends User = User>(opts: Auth0ProviderOptions<TUser>) => {
    const { children, skipRedirectCallback, onRedirectCallback = defaultOnRedirectCallback, context = Auth0Context, onClientCreated, ...clientOpts } = opts;
    const [client] = useState(() => {
        const createdClient = new Auth0Client(toAuth0ClientOptions(clientOpts));

        if (opts.onClientCreated) {
            return opts.onClientCreated(createdClient);
        }
    ...
    }, [client, onRedirectCallback, skipRedirectCallback, handleError, onClientCreated]);
    ...
    const getDpopNonce = useCallback<Auth0Client['getDpopNonce']>((id) => client.getDpopNonce(id), [client]);

    const setDpopNonce = useCallback<Auth0Client['setDpopNonce']>((nonce, id) => client.setDpopNonce(nonce, id), [client]);

    const generateDpopProof = useCallback<Auth0Client['generateDpopProof']>((params) => client.generateDpopProof(params), [client]);

    const createFetcher = useCallback<Auth0Client['createFetcher']>((config) => client.createFetcher(config), [client]);
