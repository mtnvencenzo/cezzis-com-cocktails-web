...
resource "azuread_application" "cocktails_web_app_registration" {
...
resource "azuread_service_principal" "cocktails_web_app_registration_serviceprincipal" {
  client_id                    = azuread_application.cocktails_web_app_registration.client_id
  app_role_assignment_required = false
  description                  = "Service principal for the ${azuread_application.cocktails_web_app_registration.display_name}"
}

resource "azuread_app_role_assignment" "cocktails_web_app_registration_serviceprincipal_app_rw_all_role_assignment" {
  app_role_id         = azuread_service_principal.msgraph.app_role_ids["Application.ReadWrite.All"]
  principal_object_id = azuread_service_principal.cocktails_web_app_registration_serviceprincipal.object_id
